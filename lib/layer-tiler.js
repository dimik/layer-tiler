...
var LayerTiler = module.exports = inherit(/** @lends LayerTiler prototype. */ {
    ...
    renderTilesAtZoom: function (zoom) {
        var tilesBounds = this.getTilesBounds(zoom),
            handlers = [];

        for(var x = tilesBounds[0][0]; x <= tilesBounds[1][0]; x++) {
            for(var y = tilesBounds[1][1]; y <= tilesBounds[0][1]; y++) {
                handlers.push(
    ...
    getTileUrl: function (x, y, zoom) {
        ...
        return util.format(path.join(options.output, options.tileUrlTemplate), zoom, x, y);
    },
    /**
     * Default options.
     * @function
