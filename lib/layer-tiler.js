...
var LayerTiler = module.exports = inherit(/** @lends LayerTiler prototype. */ {
    ...
    _createFolder: function (name) {
        ...
        fs.mkdir(path.resolve(path.join(__dirname, name)), function (err) {
            if(err) {
                promise.reject(err);
            }
            else {
    ...
    getTileUrl: function (x, y, zoom) {
        ...
        return util.format(path.join(__dirname, options.tileUrlTemplate), zoom, x, y, options.tileType);
    },
    /**
     * Default options.
     * @function
